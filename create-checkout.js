...
module.exports = async function handler(req, res){
  ...
  try {
    ...
    if(/testmode|live mode|test mode/i.test(m)){
      return res.status(500).json({
        error: 'Test and live mode are mixed up.',
        fix: 'Your secret key and your price id must be from the same mode. Check both in Stripe with the ' +
             'test-mode toggle set the same way.'
      });
    }
