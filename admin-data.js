  ...
  try {
    ...
    if(!await isAdmin(user.id)){
      return res.status(403).json({
        error: 'Signed in, but this account is not an administrator. ' +
               'Run: update profiles set is_admin = true where email = \'' + (user.email || '') + '\';'
      });
    }
    ...
    // A missing variable is the usual cause and is worth naming, so the fix
    // is obvious rather than a guess. The value is never returned.
    if(err.missing){
      return res.status(500).json({
        error: 'Not configured: ' + err.missing + ' is not set in Vercel.',
        missing: err.missing
      });
    }
    return res.status(500).json({ error: 'Could not load: ' + err.message });
  }
};
